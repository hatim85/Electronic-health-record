...
const registerUser = async (enrollId, userID, userRole, args) => {
  ...
  if (!orgID) {
    throw new Error(
      `Invalid user role: ${userRole}. Please check roleToOrg mapping.`
    );
  }
  ...
  if (existing) {
    console.log(
      `⚠️ Identity for ${userID} already exists. If it was created without attrs, delete it and re-run.`
    );
  } else {
    ...
    if (!adminIdentity) {
      throw new Error(
        `Admin identity ${adminID} not found in wallet. Run enrollAdmin.js for ${orgID}`
      );
    }
    ...
    console.log(
      `✅ Registered & enrolled user ${userID} (${userRole}) into ${orgID}`
    );
  }
  ...
  if (
    ["doctor", "diagnostics", "pharmacy", "patient"].includes(
    ...
    console.log(
      "Registering insurance agent, using insuranceAdmin as submitter"
    );
    submitterIdentity = "insuranceAdmin"; // insuranceAdmin for insurance agents
  }
  else if (userRole === "hospital") {
    submitterIdentity = "hospitalAdmin"; // ✅ IMPORTANT: admin submits registerHospital
  }
  else {
  ...
  if (!submitter) {
    throw new Error(
      `Submitter identity ${submitterIdentity} not found in wallet. Ensure it is enrolled.`
    );
  }
  ...
  switch (userRole) {
    case "patient":
      console.log(
        `Registering patient ${userID} with args: ${JSON.stringify(args)}`
      );
    ...
    case "hospital":
      buffer = await contract.submitTransaction(
        console.log(`Registering hospital ${userID} with args: ${JSON.stringify(args)}`) ||
        "registerHospital",
        JSON.stringify({
          hospitalId: userID,
...
const login = async (userID, userRole) => {
  ...
  return {
    statusCode: 200,
    userID,
    message: `Login successful for ${userID} (${userRole})`,
  };
};
