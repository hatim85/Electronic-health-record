...
export default function RegisterResearcher() {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.researcherId) newErrors.researcherId = "Researcher ID is required";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ researcherId: "", name: "", institution: "" });
      setErrors({});
    } catch (err) {
      setMessage(`❌ ${err.error || "Failed to register researcher"}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-lg p-8 sm:p-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Register Researcher</h1>
          <p className="text-sm text-gray-500 mt-2 text-center">
            Add a new researcher to the system
          </p>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          {[
            { label: "Researcher ID", name: "researcherId" },
