...
export default function DispenseMedicine() {
  ...
  const validateForm = () => {
    ...
    if (!form.recordId) newErrors.recordId = "Record ID is required";
    if (!form.medicineName) newErrors.medicineName = "Medicine Name is required";
    if (!form.quantity) newErrors.quantity = "Quantity is required";
    else if (form.quantity <= 0) newErrors.quantity = "Quantity must be positive";
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ patientId: "", recordId: "", medicineName: "", quantity: "" });
      setErrors({});
    } catch (err) {
      setMessage(`❌ ${err.error || "Failed to dispense medicine"}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-lg p-8 sm:p-10">
        {/* Back button */}
        <button
          onClick={() => navigate("/pharma/dashboard")}
        ...
        <div className="flex flex-col items-center mb-8">
          <Syringe className="w-12 h-12 text-blue-600 mb-3" />
          <h1 className="text-3xl font-bold text-gray-800">Dispense Medicine</h1>
          <p className="text-sm text-gray-500 mt-2 text-center">
            Dispense medicine for a patient prescription
          </p>
        </div>
