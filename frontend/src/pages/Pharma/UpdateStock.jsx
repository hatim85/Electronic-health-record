...
export default function UpdateStock() {
  ...
  const validateForm = () => {
    ...
    if (!form.newStock) newErrors.newStock = "New Stock is required";
    else if (form.newStock < 0) newErrors.newStock = "Stock cannot be negative";
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ medicineName: "", newStock: "" });
      setErrors({});
    } catch (err) {
      setMessage(`❌ ${err.error || "Failed to update stock"}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-lg p-8 sm:p-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Update Medicine Stock</h1>
          <p className="text-sm text-gray-500 mt-2 text-center">
            Update the stock quantity for a medicine
          </p>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          {[
            ...
            { label: "New Stock", name: "newStock", type: "number" },
          ].map((field) => (
