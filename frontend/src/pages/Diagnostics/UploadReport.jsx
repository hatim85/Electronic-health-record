...
export default function UploadReport() {
  ...
  const validateForm = () => {
    ...
    if (!formData.reportData) newErrors.reportData = "Report Data is required";
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ patientId: "", reportType: "", reportData: "" });
      setErrors({});
    } catch (err) {
      setMessage(`❌ ${err.error || "Failed to upload lab report"}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-lg p-8 sm:p-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-2 text-center">
            Submit a new lab report for a patient
          </p>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          {[
            ...
            { label: "Report Type (e.g., Blood Test)", name: "reportType" },
          ].map((field) => (
          ...
          <div className="relative">
            <textarea
              ...
              onChange={handleChange}
              className={`peer w-full px-4 py-3 border ${
                errors.reportData ? "border-red-500" : "border-gray-300"
              } rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition text-gray-700 placeholder-transparent`}
            ...
            <label
              className={`absolute left-4 top-3 text-gray-500 text-sm transition-all transform peer-placeholder-shown:top-4 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400 peer-focus:top-[-8px] peer-focus:text-sm peer-focus:text-blue-600 bg-white px-1 ${
                formData.reportData ? "top-[-8px] text-sm text-blue-600" : ""
              }`}
            >
              Report Data
            </label>
            {errors.reportData && (
              <p className="text-red-500 text-xs mt-1">{errors.reportData}</p>
            )}
          </div>
