...
export default function IssueInsurance() {
  ...
  const validateForm = () => {
    ...
    } else if (isNaN(form.coverageAmount) || Number(form.coverageAmount) <= 0) {
      newErrors.coverageAmount = "Coverage Amount must be a positive number";
    }
    if (!form.insuranceCompany) newErrors.insuranceCompany = "Insurance Company is required";
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(`❌ ${err.error || "Failed to issue insurance"}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-lg p-8 sm:p-10">
        ...
        <div className="flex flex-col items-center mb-8">
          <FilePlus className="w-12 h-12 text-blue-600 mb-3" />
          <h1 className="text-3xl font-bold text-gray-800">Issue Insurance Policy</h1>
          <p className="text-sm text-gray-500 mt-2 text-center">
            Issue a new insurance policy for a patient
          </p>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          {[
            ...
            { label: "Coverage Amount", name: "coverageAmount", type: "number" },
