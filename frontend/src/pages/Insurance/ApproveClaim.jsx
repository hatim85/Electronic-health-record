...
export default function ApproveClaim() {
  ...
  const [form, setForm] = useState({ claimId: "", patientId: "" });
  const [errors, setErrors] = useState({});
  const [message, setMessage] = useState("");
  const [loading, setLoading] = useState(false);

  // Pre-fill claimId from URL params
  useEffect(() => {
    if (claimId) {
      setForm((prev) => ({ ...prev, claimId }));
    }
  }, [claimId]);
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!form.claimId) newErrors.claimId = "Claim ID is required";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ claimId: "", patientId: "" });
      setErrors({});
    } catch (err) {
      setMessage(`❌ ${err.error || "Failed to approve claim"}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-lg p-8 sm:p-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Approve Claim</h1>
