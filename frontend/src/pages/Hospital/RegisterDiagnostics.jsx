...
const RegisterDiagnostics = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await registerDiagnostic({ ...form, hospitalId: user.id });
  ...
  return (
    <div className="p-6">
      ...
      <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
        <input name="diagnosticsId" value={form.diagnosticsId} onChange={handleChange} placeholder="Diagnostics ID" className="border p-2 w-full"/>
