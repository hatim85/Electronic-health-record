...
const RegisterPatient = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await registerPatient({ ...form, hospitalId: user.id });
  ...
  return (
    <div className="p-6">
      ...
      <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
        <input name="patientId" value={form.patientId} onChange={handleChange} placeholder="Patient ID" className="border p-2 w-full"/>
        ...
        <input name="gender" value={form.gender} onChange={handleChange} placeholder="Gender" className="border p-2 w-full"/>
