...
async function addRecord(ctx, args) {
    ...
    if (!patient.authorizedEntities || !patient.authorizedEntities.includes(callerId)) {
        throw new Error(`Doctor ${callerId} is not authorized for patient ${args.patientId}`);
    }

    // create record id and composite key per patient
    const txId = ctx.stub.getTxID();
    const recordId = `R-${txId}`;
    const recordKey = ctx.stub.createCompositeKey('record', [args.patientId, recordId]);
    ...
    return _stringify({ success: true, message: `Record ${recordId} added`, recordId });
}
...
async function updatePatientRecord(ctx, args) {
    ...
    // Fetch the record
    const recordKey = ctx.stub.createCompositeKey('record', [args.patientId, args.recordId]);
    const recordBytes = await ctx.stub.getState(recordKey);
    if (!recordBytes || recordBytes.length === 0) {
        throw new Error(`Record ${args.recordId} not found for patient ${args.patientId}`);
    }
    ...
    if (record.doctorId !== callerId) {
        throw new Error(`Doctor ${callerId} is not the owner of record ${args.recordId}`);
    }

    // Update fields
    if (args.diagnosis) record.diagnosis = args.diagnosis;
    if (args.prescription) record.prescription = args.prescription;
    record.updatedAt = new Date(ctx.stub.getTxTimestamp().seconds.low * 1000).toISOString();
...
async function uploadPatientDescription(ctx, args) {
    ...
    const txId = ctx.stub.getTxID();
    const descId = `D-${txId}`;
    const descKey = ctx.stub.createCompositeKey('description', [args.patientId, descId]);
