...
async function grantAccess(ctx, args) {
    ...
    if (!Array.isArray(patient.authorizedEntities)) patient.authorizedEntities = [];
    ...
    if (!patient.authorizedEntities.includes(args.entityId)) {
        ...
        patient.updatedAt = new Date(txTimestamp.seconds.low * 1000).toISOString();

        await ctx.stub.putState(patientKey, Buffer.from(_stringify(patient)));
    }

    // Create consent record for entity
    const consentKey = ctx.stub.createCompositeKey('consent', [args.patientId, args.entityId]);
    const consentRecord = {
    ...
    if (rewardableRoles.includes(args.entityRole)) {
        await this.creditReward(ctx, { patientId: args.patientId, points: 10 }); // adjust points as needed
    }
...
async function useReward(ctx, args) {
    args = typeof args === 'string' ? JSON.parse(args) : args;
    const { role, uuid } = getCallerAttributes(ctx);
    if (role !== 'patient') throw new Error('Only patient can use reward tokens');

    if (!args.patientId || !args.amount) throw new Error('patientId and amount required');
    if (uuid !== args.patientId) throw new Error('Patient can only use own rewards');
    ...
    if (rewardBytes && rewardBytes.length > 0) rewardData = JSON.parse(rewardBytes.toString());

    const amount = parseInt(args.amount, 10);
    if (isNaN(amount) || amount <= 0) throw new Error('Invalid amount');

    if (rewardData.balance < amount) throw new Error(`Insufficient reward balance: ${rewardData.balance}`);
    ...
    return _stringify({ success: true, message: 'Reward used', newBalance: rewardData.balance });
}
